...
public class ManagerBase : MonoBase {
    ...
    /// <param name="message">具体参数.</param>
    public override void Execute(int eventCode,params object[] message)
    {
        if (!dict.ContainsKey(eventCode))//如果没有注册退出方法
        {
            Debug.LogWarning(GetType().Name + "没有注册 ： " + eventCode);
            return;
        }
    ...
    public void Remove(int eventCode, MonoBase mono)
    {
        ...
        if (!dict.ContainsKey(eventCode))
        {
            Debug.LogWarning(GetType().Name + "没有这个事件" + eventCode + "注册");
            return;
        }
